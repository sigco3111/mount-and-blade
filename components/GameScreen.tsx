...
interface GameScreenProps {
  ...
  isDelegated: boolean;
  tokenUsage: { total: number, session: number, last: number };
        ...
        <div className="flex flex-col items-center space-y-6 p-4">
            ...
            <div className="w-full max-w-sm space-y-4">
                ...
                <button
                 ...
                 <button
                  onClick={onHealParty}
                  disabled={combinedIsLoading || !isAnyoneWounded || player.gold < 100}
                  title={!isAnyoneWounded ? "부상당한 아군이 없습니다." : player.gold < 100 ? "비용이 부족합니다 (100G)." : "편안한 침대에서 휴식을 취해 체력을 회복합니다."}
                  className="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold py-3 px-4 rounded-lg disabled:bg-slate-600 disabled:cursor-not-allowed transition duration-200 flex items-center justify-center space-x-2"
                >
                  <BedIcon className="w-5 h-5"/>
                  <span>편안한 휴식 (100G)</span>
                </button>
            </div>
    ...
    <div className="min-h-screen bg-slate-900 text-slate-200 p-4 sm:p-6 lg:p-8 font-sans" style={{backgroundImage: "linear-gradient(rgba(20,30,48,0.9), rgba(36,59,85,0.9)), url('https://picsum.photos/seed/worldmap/1920/1080')", backgroundAttachment: 'fixed'}}>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-slate-800/70 backdrop-blur-sm border border-slate-700 rounded-lg p-6 shadow-lg">
             <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex items-center space-x-1">
                    ...
                    <button onClick={() => setShowHelpModal(true)} title="도움말" className="p-2 rounded-full text-slate-400 hover:bg-slate-700 hover:text-white transition-colors">
          ...
          <TokenUsageDisplay total={tokenUsage.total} session={tokenUsage.session} last={tokenUsage.last} />
        </div>
